...
// New returns a domainutils struct
func New(domain string) domainutils {
	return domainutils{domain: domain}
}

// IsValid checks if a given string fits a domain schemata
func (d *domainutils) IsValid() (bool, error) {
	r, err := regexp2.Compile(`^((?!-))(xn--)?[a-z0-9][a-z0-9-_]{0,61}[a-z0-9]{0,1}\.(xn--)?([a-z0-9\-]{1,61}|[a-z0-9-]{1,30}\.[a-z]{2,})$`, 0)
	if err != nil {
		return false, err
	}
	match, err := r.MatchString(d.domain)
	if err != nil {
		return false, err
	}
	if !match {
...
func (d *domainutils) ExtractTld() (string, error) {
	r, err := regexp2.Compile(`[^.]+$`, 0)
	if err != nil {
		return "", err
	}
	tld, err := r.FindStringMatch(d.domain)
	if err != nil {
		return "", err
	}
	if tld == nil {
		return "", errors.New("TLD Empty")
	}
...
func (d *domainutils) IsValidTld(tld string) (bool, error) {
...
// HasValidTld checks a domain/url/uri against a given list of TLD from iana
func (d *domainutils) HasValidTld() (bool, error) {
	tld, err := d.ExtractTld()
	if err != nil {
		return false, err
	}
	return d.IsValidTld(tld)
}
